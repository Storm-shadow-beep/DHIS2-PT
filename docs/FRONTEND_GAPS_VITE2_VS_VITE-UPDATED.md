...
- `controllers/authController.js` + `routes/authRoutes.js`: differ (JWT/OTP vs simple MySQL). Neither is used directly by `vite-2` at runtime (backend is `auth-server/src/`); treat as reference only.
