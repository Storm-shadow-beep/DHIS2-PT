...
- `server/index.js` (UPDATED): `GET /api/health|me|users|projects|projects/:id/phases|projects/:id/documents|reports`, `POST /api/register|login|logout|projects|projects/:id/publish|unpublish|projects/:id/documents|reports`, `PATCH /api/projects/:id|.../phases/:phaseId|documents/:id/review`, `PUT /api/projects/:id/members`, with `requireAuth/requireManager` + `authLimiter 10/15m`. As of 2026-09-11 `auth-server` has `auth.routes.ts` + `admin.routes.ts` + `project.routes.ts` + `project-users.routes.ts` (project CRUD guarded by `requirePermission` + project guards) — document/phase/publish/report routes are still missing and must be built with Drizzle/Postgres + `permission.middleware` (cannot copy `server/index.js` verbatim: MySQL vs `pg`, cookie `remember_me` vs JWT+refresh, no OTP). See `AUTHENTICATION_AUTHORIZATION_CURRENT.md §3`.
