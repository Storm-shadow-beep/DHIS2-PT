...
async function getRecentSendStats(
  ...
  const emailRows = await db
    ...
    .from(otpChallenges)
    .where(and(eq(otpChallenges.email, email), eq(otpChallenges.purpose, OTP_PURPOSE), gt(otpChallenges.createdAt, since)))
    .orderBy(asc(otpChallenges.createdAt));
  const ipRows = ipAddress
    ? await db
      .select({ id: otpChallenges.id, createdAt: otpChallenges.createdAt })
      .from(otpChallenges)
      .where(and(eq(otpChallenges.ipAddress, ipAddress), eq(otpChallenges.purpose, OTP_PURPOSE), gt(otpChallenges.createdAt, since)))
      .orderBy(asc(otpChallenges.createdAt))
    : [];
...
async function createChallenge(identity: LoginIdentity, metadata: ChallengeMetadata): Promise<{
  challenge: ChallengeResponse;
  code: string;
}> {
  const now = new Date();
  const recentSends = await getRecentSendStats(identity.user.email, metadata.ipAddress, now);
  ...
  const challenge = await db.transaction(async (tx) => {
    await tx
      ...
      .where(and(eq(otpChallenges.userId, identity.user.id), eq(otpChallenges.purpose, OTP_PURPOSE), isNull(otpChallenges.consumedAt), isNull(otpChallenges.supersededAt)));
...
export async function createLoginChallenge(identity: LoginIdentity, metadata: ChallengeMetadata): Promise<ChallengeResponse> {
  const result = await createChallenge(identity, metadata);
...
export async function resendLoginChallenge(challengeId: string, metadata: ChallengeMetadata): Promise<ChallengeResponse> {
  ...
  const [current] = await db
    .select()
    .from(otpChallenges)
    .where(and(eq(otpChallenges.id, challengeId), eq(otpChallenges.purpose, OTP_PURPOSE)))
    .limit(1);
  ...
  return (await createChallenge(identity, metadata)).challenge;
}
