...
export const DocumentsPage: React.FC = () => {
  ...
  const isTeamMember = hasRole(user, ROLE_NAMES.TEAM_MEMBER);
  ...
  const loadProject = async (projectId: string) => {
    ...
    const nextOpenPhases = nextPhases.filter((phase) => phase.status !== 'completed');
    setUploadForm((current) => ({
      ...current,
      phaseId: nextOpenPhases.some((phase) => phase.id === current.phaseId)
        ? current.phaseId
        : nextOpenPhases[0]?.id ?? '',
    }));
  };
  ...
  const openPhases = useMemo(() => phases.filter((phase) => phase.status !== 'completed'), [phases]);
  const canSubmitDocuments = canEditDocuments && (!isTeamMember || openPhases.length > 0);
  const activeDocumentPhase = phases.find((phase) => phase.id === activeDocument?.phaseId);
  const canEditActiveDocument = canEditDocuments
    && (!isTeamMember || activeDocumentPhase?.status !== 'completed');
  ...
  const handleUpload = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setUploadForm({ name: '', phaseId: openPhases[0]?.id ?? '' });
    ...
    <div className="documents-page">
      <div className="documents-header">
        ...
        <div className="documents-header-actions">
          {canSubmitDocuments && <button className="documents-submit-button" onClick={() => setShowUploadForm((current) => !current)}>{showUploadForm ? 'Close form' : '+ Submit new file'}</button>}
      ...
      {canEditActiveDocument && showEditForm && activeDocument && <div className="submit-document-panel">
        ...
        <form className="submit-document-form" onSubmit={handleSaveEdit}>
          <div className="upload-form-grid">
            ...
            <label className="upload-field"><span>Phase</span><select required value={editForm.phaseId} onChange={(event) => setEditForm((current) => ({ ...current, phaseId: event.target.value, documentCategoryId: '' }))}>{openPhases.map((phase) => <option key={phase.id} value={phase.id}>{phase.displayName || phase.name}</option>)}</select></label>
      ...
      {isTeamMember && openPhases.length === 0 && <p className="documents-read-only-notice" role="status">All project phases are complete. Team members can view documents but cannot submit new files.</p>}
      {canSubmitDocuments && showUploadForm && <div className="submit-document-panel">
        ...
        <form className="submit-document-form" onSubmit={handleUpload}>
          <div className="upload-form-grid">
            ...
            <label className="upload-field"><span>Phase</span><select required value={uploadForm.phaseId} onChange={(event) => setUploadForm((current) => ({ ...current, phaseId: event.target.value }))}>{openPhases.map((phase) => <option key={phase.id} value={phase.id}>{phase.displayName || phase.name}</option>)}</select></label>
          </div>
